...
public class Kernel {
    ...
    */
    ...
    /** System call to return current time
     */
    ...
    public static final int SYSCALL_END_IO = 7;
    ...
    */
    ...
    private static int doGetDiskSize() {
        return disk.DISK_SIZE;
    }

    private static int doRead(int blockNumber, byte[] buffer) {
        Library.output("Kernel: Reading block "+blockNumber+"\n");
        return bufferPool.read(blockNumber,buffer);
    }
    
    private static int doWrite(int blockNumber, byte[] buffer) {
        Library.output("Kernel: Writing block "+blockNumber+"\n");
        return bufferPool.write(blockNumber,buffer);
    }
    
    private static int doEndIO() {
        Library.output("INTERRUPT\n");
        return bufferPool.endIO();
    } 
    /** Performs the time check operation
     */
